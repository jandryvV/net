      ...
      <div class="mb-6">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div class="breadcrumbs text-sm">
            <ul>
              <li>
                <router-link to="/">{{ t('projectDetail.breadcrumb.home') }}</router-link>
              </li>
              <li>{{ project.title }}</li>
            </ul>
          </div>
            ...
            <div class="card-body">
              ...
              <div v-if="project.tags?.length" class="mb-6">
                <h3 class="text-lg font-semibold mb-3">
                  {{ t('projectDetail.sections.technologies') }}
                </h3>
                <div class="flex flex-wrap gap-2">
                  <span v-for="tag in project.tags" :key="tag" class="badge badge-primary badge-lg">
                    {{ tag }}
                  </span>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between pt-6 border-t border-base-300">
                <div class="flex items-center gap-4">
                  ...
                  <button class="btn btn-ghost flex items-center gap-2" @click="scrollToComments">
                ...
                <div class="flex items-center gap-2">
                  <button v-if="isProjectOwner" class="btn btn-outline btn-sm" @click="editProject">
                    <PencilIcon class="h-4 w-4" />
                    {{ t('projectDetail.sections.edit') }}
                  </button>
                  <button v-if="isProjectOwner" class="btn btn-primary" @click="collaborateProject">
                    ...
                    <div class="w-8 h-8 rounded-full">
                      <img
                        :src="
                          ...
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.user?.full_name || 'Usuario')}&background=random`
                        "
                    ...
                    <div class="stats shadow">
                      <div class="stat p-4">
                        <div class="stat-title text-xs">
                          {{ t('projectDetail.tasks.stats.total') }}
                        </div>
                        <div class="stat-value text-2xl">{{ tasks.length }}</div>
                      </div>
                      <div class="stat p-4">
                        <div class="stat-title text-xs">
                          {{ t('projectDetail.tasks.stats.completed') }}
                        </div>
                      ...
                      <div class="stat p-4">
                        <div class="stat-title text-xs">
                          {{ t('projectDetail.tasks.stats.inProgress') }}
                        </div>
              ...
              <div class="space-y-3">
                <div>
                  <span class="text-sm font-medium text-base-content/60">{{
                    t('projectDetail.sidebar.created')
                  }}</span>
                ...
                <div>
                  <span class="text-sm font-medium text-base-content/60">{{
                    t('projectDetail.sidebar.updated')
                  }}</span>
                  <p class="text-sm">{{ formatDate(project.updated_at) }}</p>
                </div>

                <div v-if="teamMembers.length > 0">
                  <span class="text-sm font-medium text-base-content/60 mb-3 block"
                    >{{ t('projectDetail.sidebar.team') }} ({{ teamMembers.length }}):</span
                  >
                  <div class="space-y-2">
                    <div
          ...
          <div class="card bg-base-100 shadow-lg">
            <div class="card-body">
              ...
              <div v-else class="text-sm text-base-content/60">
                {{ t('projectDetail.sidebar.historyEmpty') }}
              </div>
            </div>
          </div>
              ...
              <div v-if="relatedProjects.length > 0" class="space-y-3">
                ...
                >
                  ...
                  <p class="text-xs text-base-content/60">
                    {{ getProjectCreatorName(relatedProject) }}
                  </p>
    ...
    <div v-if="showDeleteCommentModal" class="modal modal-open">
      <div class="modal-box">
        ...
        <div class="modal-action">
          ...
          <button class="btn btn-error" @click="deleteComment">
...
const historyEntries = computed(() => {
  ...
  if (project.value.created_at) {
    entries.push({
      title: t('projectDetail.sidebar.historyCreated'),
      detail: `${t('projectDetail.sidebar.historyCreatedBy')} ${getProjectCreatorName(project.value)}`,
      time: formatDate(project.value.created_at),
    })
  }
  if (project.value.updated_at && project.value.updated_at !== project.value.created_at) {
    entries.push({
      title: t('projectDetail.sidebar.historyUpdated'),
      detail: `${t('projectDetail.sidebar.historyUpdated')}`,
      time: formatDate(project.value.updated_at),
    })
  }
  return entries
})
...
const saveEditComment = async () => {
  ...
  try {
    const updatedComment = await projectsStore.updateComment(
